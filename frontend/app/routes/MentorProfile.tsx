    ...
    return (
      <div className='py-8'>
        <EmptyState
          ...
          description='Hồ sơ này hiện không có sẵn hoặc đã được gỡ khỏi danh sách công khai.'
    ...
    <div className='grid gap-8 py-6 lg:grid-cols-[minmax(0,1.15fr)_360px]'>
      ...
      <div className='space-y-6'>
        ...
        <section className='rounded-3xl border border-slate-200 bg-white p-6'>
          <SectionTitle
            size='md'
            subtitle='Chọn đúng môn học và cấp lớp để gửi yêu cầu đặt lịch sát với nhu cầu thực tế.'
        ...
        <section className='grid gap-6 xl:grid-cols-2'>
          ...
          <div className='rounded-3xl border border-slate-200 bg-white p-6'>
            <SectionTitle
              size='md'
              subtitle='Xem nhanh những yếu tố giúp phụ huynh và học viên đánh giá độ phù hợp.'
              title='Điểm tin cậy'
            />
            <div className='mt-5 space-y-3'>
              <TrustRow
                description='Hồ sơ công khai sau khi được đội ngũ kiểm duyệt.'
                kind='approval'
                status={mentor.approvalStatus}
                title='Trạng thái duyệt mentor'
              />
              <TrustRow
                description='Xác minh danh tính được theo dõi riêng với trạng thái duyệt hồ sơ.'
                kind='verification'
...
type TrustRowProps = {
  kind: 'approval' | 'verification'
  status: Mentor['approvalStatus'] | Mentor['verificationStatus']
  title: string
  description: string
}

function TrustRow({ description, kind, status, title }: TrustRowProps) {
  return (
    <div className='rounded-2xl border border-slate-200 bg-slate-50 p-4'>
...
function ClockIcon() {
  return <CalendarDays className='text-primary h-4 w-4' />
}
