...
const BookingSidebar = ({ mentor }: BookingSidebarProps) => {
  const activeOffering = mentor.offerings.find((offering) => offering.active) ?? mentor.offerings[0]
  const nextWindows = mentor.specificDateAvailability.slice(0, 2)

  return (
    <aside className='shadow-soft sticky top-24 rounded-3xl border border-slate-200 bg-white p-5'>
      <div className='flex items-center justify-between gap-3'>
        <div>
          <p className='text-muted text-sm'>Bắt đầu từ</p>
      ...
      <div className='mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4'>
        <p className='text-ink text-sm font-semibold'>Buổi học được gợi ý</p>
      ...
      <div className='mt-6 rounded-2xl bg-slate-50 p-4 text-sm'>
        ...
        <div className='text-muted mt-2 flex items-center justify-between'>
          <span>Hình thức</span>
          <span>{mentor.meetingTypes[0]}</span>
        </div>
